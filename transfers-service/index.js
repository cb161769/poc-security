...
async function getPubKeyBinding(sub) {
  try {
    const d = await redis.get(`pubkey:${sub}`);
...
const odooBreaker = new CircuitBreaker(callOdoo, {
  timeout: 3000, errorThresholdPercentage: 50, resetTimeout: 30000, volumeThreshold: 3, name: 'odoo-transfers',
});
odooBreaker.on('open',  () => console.warn('[circuit:odoo-transfers] OPEN'));
odooBreaker.on('close', () => console.info('[circuit:odoo-transfers] CLOSED'));
...
async function validateClientInOdoo(req, res, next) {
  try {
    ...
    console.error('[transfers] Error Odoo:', error.message);
...
app.post('/', validateJWT, validateClientInOdoo, validatePubKeyBinding, decryptRequestBody, checkIdempotency, async (req, res) => {
  ...
  const MAX_AMOUNT = parseFloat(process.env.MAX_TRANSFER_AMOUNT || '10000');
